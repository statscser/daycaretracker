    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
          <span style={{ fontSize:16, fontWeight:800, color:M.char }}>{s.settingsTitle}</span>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:12, fontWeight:700, color:M.lChar, display:"block", marginBottom:6 }}>{s.settingsLang}</label>
          <div style={{ display:"flex", gap:6 }}>
            {[{ code:"zh", label:"🇨🇳 中文" }, { code:"en", label:"🇺🇸 English" }].map(l => (
              <button key={l.code} onClick={() => onUpdateSettings({ lang: l.code })} style={{
                padding:"6px 14px", borderRadius:14, fontSize:12, fontWeight:700,
                fontFamily:"inherit", cursor:"pointer", transition:"all 0.15s",
                border: lang === l.code ? `2px solid ${M.sageDk}` : `1.5px solid ${M.brown}30`,
                background: lang === l.code ? `${M.sage}30` : M.white, color:M.char,
              }}>{l.label}</button>
            ))}
          </div>
        </div>
        ...
        {/* Currency */}
        <div style={{ marginBottom:16 }}>
          <label style={{ fontSize:12, fontWeight:700, color:M.lChar, display:"block", marginBottom:6 }}>{s.settingsCurrency}</label>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
            {sorted.map((entry, i) => (
              <div key={`${entry.startDate}-${i}`} style={{
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:10, color:M.lChar, fontWeight:600 }}>{s.settingsFrom} {entry.startDate}</div>
                  <div style={{ fontSize:15, fontWeight:800, color:M.char }}>
                    {fmt(entry.amount)}
                    <span style={{ fontSize:10, fontWeight:600, color:M.lChar, marginLeft:3 }}>{s.settingsPerMo}</span>
                  </div>
                </div>
                {sorted.length > 1 && (
          ...
          {!showAdd ? (
            ...
            <div style={{
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:8, marginBottom:8 }}>
                <div>
                  <label style={{ fontSize:10, fontWeight:700, color:M.lChar, display:"block", marginBottom:4 }}>{s.settingsDateLbl}</label>
        ...
        <div style={{ display:"flex", gap:12 }}>
          {[
            { label:s.settingsDropoff, val:sh, key:"sh", opts:Array.from({length:12},(_,i)=>i+6) },
            { label:s.settingsPickup,  val:eh, key:"eh", opts:Array.from({length:12},(_,i)=>i+12) },
          ].map((x, i) => (
