...
const HUMOR_USD = [
  ...
  (c) => `≈ ${Math.max(1,Math.floor(c/15))} 箱 Costco 尿不湿 🧷`,
  (c) => `≈ ${Math.max(1,Math.floor(c/35))} 罐 Enfamil 奶粉 🍼`,
  (c) => `≈ ${Math.max(1,Math.floor(c/2.5))} 个 Trader Joe's 鲜花卷 🥐`,
  (c) => `≈ ${Math.max(1,Math.floor(c/12))} 次 Target 冲动消费 🎯`,
  (c) => `≈ ${Math.max(1,Math.floor(c/200))} 个月 Disney+ 订阅 📺`,
  (c) => `多运 ${Math.max(1,Math.floor(c/0.75))} 次洗衣机而已 🧺`,
];
const HUMOR_CAD = [
...
const HUMOR_CNY = [
  (c) => `≈ ${Math.max(1,Math.floor(c/30))} 杯瑞幸椰乳拿铁 ☕`,
  (c) => `≈ ${Math.max(1,Math.floor(c/25))} 顿美团外卖 🍜`,
  (c) => `≈ ${Math.max(1,Math.floor(c/280))} 罐飞鹤奶粉 🍼`,
  (c) => `≈ ${Math.max(1,Math.floor(c/3))} 包花王纸尿裤 🧴`,
...
export default function App() {
  ...
  // Re-randomise humor when month changes or absence data appears
  ...
  return (
    <div style={{
      ...
      {/* ── Settings Modal ── */}
      {setup && <>
        <div onClick={() => setSetup(false)} style={{
        ...
        <div style={{
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
            ...
            <button onClick={() => setSetup(false)} style={{
      ...
      <div ref={calRef} style={{
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12, padding:"0 4px" }}>
          <button onClick={prev} style={{ background:"none",border:"none",fontSize:20,cursor:"pointer",padding:"4px 8px",color:M.lChar,fontFamily:"inherit" }}>‹</button>
          <div>
          ...
          <button onClick={next} style={{ background:"none",border:"none",fontSize:20,cursor:"pointer",padding:"4px 8px",color:M.lChar,fontFamily:"inherit" }}>›</button>
        </div>
